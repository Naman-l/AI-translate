import React from "react";
import Card from "./Card";
import { Box } from "@mui/material";

const MainScreen = () => {
  const products = [
    {
      title: "title",
      img: "data:image/png;base64,",
      desc: "fjcjfbcjsbchecbhe cefbcjefdbcjef cebcnebcne",
    },
  ];

//   transaltePDf
// onclick navigate to translate route

//   summarizePDF

//   TranslateImage

//   Unlock PDF

  const renderCard = (p) => {
    return <Card cardInfo= {p} />;
  };
  return <Box sx={{display:'flex',flexWrap:'wrap'}}>{products.map((e) => renderCard(e))}</Box>
};

export default MainScreen;
